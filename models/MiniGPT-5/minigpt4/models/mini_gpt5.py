...
class MiniGPT5(MiniGPT4):
    ...
    def __init__(self, *args, **kwargs):
        ...
        output_embed_grad_mask[:-self.num_new_tokens] = 0.05
        ...
        if self.using_lora:
            ...
            self.llama_model.base_model.model.model.embed_tokens.weight.requires_grad = True
            self.llama_model.base_model.model.lm_head.weight.requires_grad = True
        else:
    ...
    def forward(self, input_ids, labels, attention_mask, input_images=None, input_img_features=None,
                output_hidden_states=True):
        batch_size = input_ids.shape[0]
        all_input_embeds, all_attention, all_labels = [], [], []
        for b in range(batch_size):
            if input_img_features is not None:
                wrapped_img_embeds, wrapped_atts_img, wrapped_labels = self.input_warp(input_ids[b:b + 1],
                                                                                       attention_mask[b:b + 1],
                                                                                       labels[b:b + 1],
                                                                                       input_image_feature=
                                                                                       input_img_features[b])
            elif input_images is not None:
                wrapped_img_embeds, wrapped_atts_img, wrapped_labels = self.input_warp(input_ids[b:b + 1],
                                                                                       attention_mask[b:b + 1],
                                                                                       labels[b:b + 1], input_images[b])
            else:
                wrapped_img_embeds, wrapped_atts_img, wrapped_labels = self.input_warp(input_ids[b:b + 1],
                                                                                       attention_mask[b:b + 1],
                                                                                       labels[b:b + 1])
        ...
        for i in range(len(all_input_embeds)):
            if all_input_embeds[i].shape[1] < max_len:
                ...
                pad_embeds = torch.zeros([all_input_embeds[i].shape[0], pad_len, all_input_embeds[i].shape[2]]).to(
                    all_input_embeds[i].device)
                pad_atts = torch.zeros([all_attention[i].shape[0], pad_len]).to(all_attention[i].device)
                pad_labels = torch.ones([all_labels[i].shape[0], pad_len], dtype=torch.long).to(
                    all_labels[i].device) * -100
    ...
    def predict(self, instruction, input_image, max_new_tokens=300, num_beams=1, min_length=1, top_p=0.9,
                repetition_penalty=1.0, length_penalty=1, temperature=1.0, task_name=None, output_hidden_states=True,
        ...
        if self.Qformer is None:
            stop_words_ids = [torch.tensor([2]).to(self.device)]  # '###' can be encoded in two different ways.
        else:
    ...
    def prepare_inputs_for_generation(self, input_ids, inputs_embeds=None, force_generation=None, *args, **kwargs):
        model_kwargs = self.base_model_prepare_inputs_for_generation(input_ids, inputs_embeds=inputs_embeds, *args,
                                                                     **kwargs)
        ...
        if 'input_ids' in model_kwargs:
            ...
            if new_token_ids == self.output_img_id:
                ...
                all_img_tokens = torch.tensor(self.llama_tokenizer.convert_tokens_to_ids(ALL_IMG_TOKENS)).unsqueeze(
                    0).to(device)
                all_img_tokens_mask = torch.ones_like(all_img_tokens)[:, :IMG_TOKEN_NUM - 1].to(device)
                all_img_position_ids = torch.arange(current_position_ids,
                                                    current_position_ids + IMG_TOKEN_NUM).unsqueeze(0).to(device)
    ...
    def reset_embeddings(self):
        with torch.no_grad():
            if self.using_lora:
                embed_param = self.llama_model.base_model.model.model.embed_tokens.weight
                if embed_param.grad is not None:
                    embed_param.grad = embed_param.grad * self.input_embed_grad_mask

                lm_head_param = self.llama_model.base_model.model.lm_head.weight
                if lm_head_param.grad is not None:
                    lm_head_param.grad = lm_head_param.grad * self.output_embed_grad_mask
            else:
