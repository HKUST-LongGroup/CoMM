...
def decode_wiki_pair_for_llm(item, tokenizer=None, max_length=2048, caption_prompt=None, reverse_ratio=0.0):
    def get_image_id(tmp_cur_content):
        image_path = tmp_cur_content["image_path"]
        image_id = image_path.split(".")[0]
        return torch.load(f"../datasets/image_ids/{image_id}.pth")
    ...
    if key.endswith(".pkl"):
        try:
            ...
            cur_task_type = random.choice(task_type_list)
            sample = pickle.load(value)
            cur_data = sample
            # title = sample['title']
                ...
                for cur_step_num, cur_step in enumerate(step_info):
                    ...
                    for cur_content in cur_step:
                        if cur_content["type"] == "text":
                            ...
                            try:
                                image_id_list = get_image_id(cur_content)
                            except:
                                return key, None
                ...
                for cur_step_num, cur_step in enumerate(step_info):
                    ...
                    for cur_content in cur_step:
                        if cur_content["type"] == "text":
                            ...
                            image_tokens = BOI_TOKEN + ''.join([IMG_TOKEN.format(int(item)) for item in image_id_list]) + EOI_TOKEN
                            target_answer += image_tokens
                question = s_token + " " + input_text + sep + random.choice(prompts_out) + e_token
                answer = target_answer
        ...
        if len(a_ids) <= 0 or len(q_ids) >= max_length - 64:
            # print(f"{data_id}. error length {len(a_ids)} {len(q_ids)}")
            return key, None
        labels_item = [-100] * len(q_ids) + a_ids
        input_ids_item = q_ids + a_ids
        input_ids.extend(input_ids_item)
...
def build_wiki_datapipes_for_llm(data_dir,
                                 tokenizer=None,
                                 max_length=2048,
                                 reverse_ratio=0.5,
                                 recursive=True,
    ...
    decode_partial = functools.partial(decode_wiki_pair_for_llm,
                                       tokenizer=tokenizer,
                                       max_length=max_length,
                                       caption_prompt=caption_prompt,
                                       reverse_ratio=reverse_ratio)
...
def build_multi_datapipes(datapipes, tokenizer=None, concat_type='sample', sample_weights=None):
    assert concat_type in ['concat', 'mux_longest', 'sample', 'only_one']
    ...
    elif concat_type == 'only_one':
        datapipe = datapipes[0]
    else:
        raise NotImplementedError
    # datapipe = datapipe.set_length(17000)
    # print(len(datapipe))
    return datapipe
